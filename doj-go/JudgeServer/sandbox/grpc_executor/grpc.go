...
import (
	...
	"doj-go/JudgeServer/sandbox/model"
...
// New creates grpc_judge executor server
...
func (e *execServer) CopyFileIn(c context.Context, fileCopy *pb.FileCopy) (*pb.FileID, error) {
	w, err := e.fs.New()
	if err != nil {
	...
	fid, err := e.fs.Add(filepath.Base(fileCopy.Path), w.Name())
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
	return &pb.FileID{FileID: fid}, nil
}
func (e *execServer) CopyFileOut(c context.Context, fileCopy *pb.FileCopy) (*emptypb.Empty, error) {
	_, file := e.fs.Get(fileCopy.GetFileID())
	if file == nil {
		return nil, status.Errorf(codes.NotFound, "file %v not found", fileCopy.GetFileID())
	}
	...
	defer r.Close()
	err = os.MkdirAll(filepath.Dir(fileCopy.Path), 0777)
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
	w, err := os.Create(fileCopy.Path)
	if err != nil {
		// 如果文件存在
		if os.IsExist(err) {
			w, err = os.Open(fileCopy.Path)
		}
		if err != nil {
			return nil, status.Error(codes.Internal, err.Error())
		}
